...
```
workspace-root/
├── Database buat laragon/
│   └── logistik_db.sql    <-- SQL DATABASE UNTUK IMPORT LANGSUNG KE LARAGON
├── dokumentasi/
...
> [!IMPORTANT]
> **PERHATIAN PENTING**: Database yang harus di-import ke Laragon berada di folder **`Database buat laragon`** pada root directory proyek (yaitu file **`Database/logistik_db.sql`**). Folder database bawaan aplikasi di dalam **`logistikita/database/`** harus dibiarkan saja dan tidak boleh digunakan untuk proses import Laragon.
